...
class DashboardsController < ApplicationController
  ...
  def set_happy_things
    ...
    @happy_things_of_you_and_friends = HappyThing.includes(:reactions).where(user_id: @user_ids).order(created_at: :desc)
  ...
  def fetch_happy_things_by_time(friend_ids)
    ...
    @happy_things_one_year_ago = HappyThing.includes(:reactions).where(
  ...
  def happy_things_by_period(period, friend_ids)
    HappyThing.includes(:reactions).where(start_time: period,
                                          user_id: friend_ids).order(created_at: :desc).group_by(&:user)
  end

  def set_happy_things_of_today
    today = Date.today
    friend_ids = with_current_user(current_user.friends_and_friends_who_added_me_ids)
    @happy_things_by_date = HappyThing.includes(:reactions).where(
